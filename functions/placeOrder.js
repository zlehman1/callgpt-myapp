async function placeOrder(functionArgs) {
  ...
  if (model?.toLowerCase().includes('pro')) {
    return { orderNumber: orderNum, price: Math.floor(quantity * 249 * 1.079) };
  } else if (model?.toLowerCase().includes('max')) {
    return { orderNumber: orderNum, price: Math.floor(quantity * 549 * 1.079) };
  }
  return { orderNumber: orderNum, price: Math.floor(quantity * 179 * 1.079) };
}
